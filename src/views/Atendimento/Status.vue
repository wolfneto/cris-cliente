<template>
  <v-container>
        ...
        <template v-slot="{ item: row, index }">
          <tr :key="index">
            ...
            >
              ...
              <div v-if="col.name == 'atendente'">
                <span v-if="row.atendente != null">{{
                  row.vendedores_dc_info.nome.toUpperCase()
                }}</span>
              </div>
      ...
      <v-list>
        <v-list-item>
          <v-btn
            ...
            dark
            color="teal"
            @click="ver_itens(objectItem)"
          >
            Ver Itens
            <v-icon dark right>fas fa-bars</v-icon>
          </v-btn>
        </v-list-item>
        <v-list-item v-if="objectItem.status == 1 && permissoes.ver_editar">
          <v-btn small class="ma-1" color="primary" @click="editar(objectItem)">
            Editar
            <v-icon dark right>far fa-edit</v-icon>
          </v-btn>
        </v-list-item>
        <v-list-item v-if="objectItem.status == 1 && permissoes.ver_copiar">
          <v-btn
            small
            class="ma-1"
            dark
            color="purple"
            @click="copiar(objectItem)"
          >
            Copiar
            <v-icon dark right>far fa-copy</v-icon>
          </v-btn>
        </v-list-item>
        <v-list-item v-if="objectItem.status == 1 && permissoes.ver_emitir">
          <v-btn
            small
            class="ma-1"
            color="primary"
        ...
        <v-list-item v-if="objectItem.status == 1 && permissoes.ver_excluir">
          <v-btn
            small
            class="ma-1"
            color="error"
        ...
        <v-list-item v-if="permissoes.ver_imprimir">
          <v-btn
            small
            class="ma-1"
            color="primary"
          ...
          <Imprimir
            :pedido="{
              cliente: objectItem.cliente_obj,
              endereco: objectItem.cliente_endereco,
              itens: objectItem.orcamento_itens,
              total: objectItem.total,
              id: objectItem.id,
    ...
    <v-dialog v-model="dialogItens" max-width="1200px">
      <v-card>
        <v-card-title>
          <span class="headline"
            >Itens do Orçamento -
        ...
        >
          <template v-slot:item.descricao="{ item }">
            <div
              :class="item.codigo != 0 ? '' : 'text-center primary white--text'"
            >
...
export default {
  ...
  methods: {
    ...
    ver_itens(pedido) {
      this.objectItens.itens = pedido.orcamento_itens;
      this.objectItens.total = pedido.total;
    ...
    async baixarPedido(item) {
      ...
      if (this.error) {
        ...
        this.numeroPedido = "PEDIDO EMITIDO: " + this.pedido;
